...
Décidément, MapBox aura été à l'honneur dans nos colonnes cette semaine. Cette fois, c'est [John Firebaugh](http://www.mapbox.com/about/team/#john-firebaugh) qui nous détaille les améliorations apportées à la nouvelle version de [ID](http://wiki.openstreetmap.org/wiki/ID), le nouvel editeur d'[OpenStreetMap](http://www.openstreetmap.org/). L'accent a été mis sur les performances (point régulièrement soulevé sur les mailings list), mais aussi sur une meilleure intégration de la barre d'outils. Bonne nouvelle également pour les aficionados des relations, celles-ci sont désormais prises en compte. Si vous êtes trop impatient pour attendre la sortie officielle, sachez que vous pouvez d'ores et déjà [essayer cette future version](http://openstreetmap.us/iD/release/#background=Bing&map=20.00/-77.02271/38.90085). Notons que l'équipe de Mapbox a travaillé avec l'équipe de FireFox, a priori l'amélioration des performances est surtout significative avec Firefox Nightly.
...
![](http://localhost/geotribu_reborn/sites/default/public/public_res/microworld_0.png){: .img-rdp-news-thumb }
