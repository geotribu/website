...
### OpenStreetMap est (enfin) en ODBL** Aussi palpitant et long qu'un feuilleton de telenovela, le changement de licence d'[OpenStreetMap](http://www.openstreetmap.org/) est sur le point d'arriver à son [épilogue](http://blog.osmfoundation.org/2012/09/11/change-to-odbl-imminent/). Concrètement qu'est ce que cela signifie ? Tout simplement que la base OpenStreetMap contient uniquement les données dont les contributeurs ont accepté le changement de licence (CC-BY-SA -> ODBL). Pour les utilisateurs et les contributeurs ce changement sera transparent, pour les [développeurs](http://wiki.openstreetmap.org/wiki/ODbL/License_Transition/Guidance_To_Data_Consumers) n'oubliez pas de vérifier vos planets générés ainsi que l'API !
