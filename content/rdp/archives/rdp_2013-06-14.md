...
Les voitures ont envahi notre environnement. Il est impossible de s'en passer sauf si vous habitez en milieu urbain avec un bon réseau de transport en commun! Nous savions déjà que le [ réseau routier](http://fr.wikipedia.org/wiki/R%C3%A9seau_routier) structurait notre territoire de manière à connecter les lieux de forte densité de population entre eux.
...
On vient de vous parler de GeoGit, restons sur le même sujet avec [GitHub](https://github.com/) qui intègre désormais en natif la visualisation des données au format [geojson](http://www.geojson.org/). Ainsi, tout fichier geojson pourra être affiché sur une carte. Côté technologies, nous avons [Leaflet](https://leafletjs.com) pour la partie carto, [MapBox](https://www.mapbox.com/) pour le rendu et [OpenStreetMap](http://www.openstreetmap.org/) pour les données. Il ne va pas sans dire que cela aura d'importantes répercussions sur nos infrastructures géographiques !
