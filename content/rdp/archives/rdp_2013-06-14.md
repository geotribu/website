...
On vient de vous parler de GeoGit, restons sur le même sujet avec [GitHub](https://github.com/) qui intègre désormais en natif la visualisation des données au format [geojson](http://www.geojson.org/). Ainsi, tout fichier geojson pourra être affiché sur une carte. Côté technologies, nous avons [Leaflet](leafletjs.com) pour la partie carto, [MapBox](http://www.mapbox.com/) pour le rendu et [OpenStreetMap](http://www.openstreetmap.org/) pour les données. Il ne va pas sans dire que cela aura d'importantes répercussions sur nos infrastructures géographiques !
