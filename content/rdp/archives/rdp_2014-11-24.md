...
Tout est dans le titre, non ? Un petit effort ? Ok, mais c'est bien parce-que c'est vous :) Cette API permet d'implémenter toutes les fonctionnalités du Géoportail national. Cette déclinaison en JavaScript est régulièrement enrichie (la dernière MAJ date du mois de mai) et cette nouvelle version enrichie les fonctionnalités liées aux données altimétriques. Pour aller plus loin, voyez le [billet de blog](http://api.ign.fr/article/422/l-api-geoportail-passe-en-version-211) et les [notes de version](http://api.ign.fr/tech-docs-js/fr/changelog.html).
