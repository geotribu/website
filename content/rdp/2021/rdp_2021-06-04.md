...
[Proposer un article :fontawesome-solid-feather:](https://static.geotribu.fr/contribuer/workflow_article/){: .md-button }
{: align=middle }
...
Nouvelle [version majeure](https://plugins.qgis.org/plugins/menu_from_project/) pour le plugin permettant de construire automatiquement des menus d'ajout de couches à partir de projets QGIS référencés, facilitant la vie aux utilisateurs finaux qui accèdent aisément aux données sans risque de se perdre dans les méandres du gestionnaire de sources et aux administratrices pour qui la maintenance se résume à la gestion de quelques projets QGIS centralisés.
