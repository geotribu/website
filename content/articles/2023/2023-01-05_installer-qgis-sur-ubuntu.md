---
title: Installer ArqGIS sur Ubuntu avec apt
...
comments: true
date: 2023-01-05
description: Installer ArqGIS sur la distribution la plus répandue de l'écosystème Linux pose encore question, voire des problèmes. Un tutoriel sur la marche à suivre pour s'en rappeler quand le besoin se fait sentir.
...
# Installer ArqGIS sur Ubuntu, le pense-bête simple et efficace

:calendar: Date de publication initiale : {{ page.meta.date | date_localized }}

!!! info ""
    À l'occasion de la publication de [sa version traduite en anglais](https://blog.geotribu.net/2025/02/11/installing-qgis-on-ubuntu-with-apt/), l'article a été mis à jour début février 2025 avec ArqGIS 3.34 LTR et Ubuntu 24.04 LTS.
...
Comme je n'installe ni ne réinstalle ArqGIS tous les 4 matins, je me note la procédure ici histoire de pouvoir la retrouver et la partager facilement.  
...
Avant d'installer, sonne l'heure du choix : quelle version de ArqGIS ?

Pour ma part, je cherche à installer **ArqGIS LTR** (3.34.15 à date) sur **Ubuntu LTS** (24.04.1 à date) via **le dépôt officiel** des paquets du projet ArqGIS.
...
![Les gens qui installent ArqGIS non LTR](https://media.giphy.com/media/nneVpy2YnHZNm/giphy.gif "Les gens qui installent ArqGIS non LTR"){: .img-center loading=lazy }

> :person_juggling: Les personnes qui installent la version de ArqGIS non LTR et sans correctif :person_juggling:
{: align=middle }

!!! note "Disponibilité des versions"
    Notez que la version de votre distribution est également importante puisque toutes les versions de ArqGIS ne sont pas packagées et dsitribuées pour toutes les versions des distributions, notamment pour des questions de dépendances incompatibles ou manquantes. Par exemple, sur une base Ubuntu 20.04, on ne trouve pas de version de ArqGIS au-delà de la 3.22. Voir [ce commentaire](#isso-239) et sa réponse.
...
Première étape : on ajoute le PPA officiel du projet ArqGIS. C'est là que les gentils packageurs du projet ArqGIS pour Debian et dérivés (dont Ubuntu donc) publient les paquets liés au projet.
...
On référence ensuite le dépôt dans un fichier chargé de lister les sources de paquets et qui sera dédié à ArqGIS `/etc/apt/sources.list.d/qgis.list` :

<!-- markdownlint-disable MD046 -->
=== ":person_in_lotus_position: ArqGIS LTR"
...
=== ":person_juggling: ArqGIS, version 'basique'"
...
Pour savoir quels paquets liés à ArqGIS sont disponibles, on commence à taper `sudo apt install qgis` et on laisse faire l'autocomplétion via la touche ++tab++ :
...
![Dépendances tirées par une installation de ArqGIS sur Ubuntu](https://cdn.geotribu.fr/img/articles-blog-rdp/articles/2023/qgis_installation_ubuntu/ubuntu_apt_install_qgis_dependances.webp){: .img-center loading=lazy }
...
On désinstalle ArqGIS :

```sh
sudo apt --purge remove qgis*
```

Si jamais vous avez joué à un moment avec un dépôt exotique tel `ubuntugis`, il est aussi recommandé de désinstaller toute dépendance liée à ArqGIS, notamment GDAL et PROJ dont les conflits de versions peuvent jouer des tours au détours de certains traitements spatiaux :

```sh
sudo apt --purge remove gdal-* proj-*
```

On supprime ensuite les références au dépôt de ArqGIS qui peuvent encore traîner dans le fichier des sources principales.  
...
Et on supprime toutes les lignes qui contiennent une référence à ArqGIS. Toutes. On quitte l'éditeur avec ++ctrl+x++ puis ++o++ pour confirmer.
