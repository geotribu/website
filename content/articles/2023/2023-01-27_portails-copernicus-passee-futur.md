...
observation de la terre. Il est responsable d'une part de la production
...
de faciliter l'utilisation des ces API dans votre language de programmation
...
Mais par contre il propose des données Sentinel-2 avec une meilleure calibration radiométrique (en utilisation la chaine de traitement [MAJA](https://www.cesbio.cnrs.fr/outils/maja/)) pour les données de niveau L2A, ainsi que des données de niveau [L3A](https://labo.obs-mip.fr/multitemp/theias-sentinel-2-l3a-monthly-cloud-free-syntheses/) qui sont des synthèses mensuelles des images sentinel L2A.
...
Certaines infrastructures geo regionales proposent également un accès local aux données Copernicus : [mviewer geobretagne](https://geobretagne.fr/pub/dreal_b/mviewer/?config=../apps/teledetection/config.xml)
...
Amazon et Google ont aussi récupéré ces donnés afin de les intégrer dans leur
...
services. 4 DIAS (Data and Information Access Service) ont alors été retenus
...
* **un stockage et un catalogue de données sentinel**. I.e une gestion d'une
...
* [Sobloo](https://sobloo.eu/index.html) : Orange Business Services (cloud), Airbus (data provider)  Capgemini
  Lead par d'Airbus. Capgemini apporte ses logiciels de traitement des données
...
Au niveau européens un cas d'application (clients) de ces offres DIAS a été entre
...
Après 4 ans de retours d'expérience sur le déploiement et l'exploitation des DIAS, leur contrat arrivant à terme, l'europe à choisi de
faire évoluer son offre de services (gratuite et payante) d'accès aux données
Sentinel.

Conrètement l'évolution consiste à selectionner un consortium (décembre 2022)
...
gratuite (avec toutefois limitation de quota), et d'autre part à offrir une
...
jusqu'a juin 2023 ou le service devra avoir atteint sa phase opérationnelle
...
6 ans avec une extention possible de 10 ans, soit possiblement une continuité
...
centaines de kilomètre ([tuiles de type MGRS](https://labo.obs-mip.fr/multitemp/the-sentinel-2-tiles-how-they-work/) de 110k0*110km pour sentinel-2) dans
...
Une première nouveauté de service devrait être l'arrivé de la mise à disposition,
...
A cela doit s'ajouter un accès aux "Copernicus Contributing Missions data"
...
A noter qu'a priori il ne devrait plus y avoir de notions de donnés online/offline
...
raster (de type calculatrice rasterà, cela permet par exemple de calculer à la
