...
Le plus dur est fait et la configuration est presque fini ! Pour vérifier que tout s'est bien passé il vous suffit maintenant de visiter la page http:// localhost/featureserver.cgi/scribble/all.atom. Si une page de flux vide s'affiche alors tout est bien configuré.

Rendez-vous maintenant sur la page d'index de featureServer (http:// localhost/featureserver/) vous devriez alors voir apparaitre l'interface d'OpenLayers avec la couche vmap0 de MetaCarta affichée avec sur la droite un formulaire permettant le paramétrage des objets géographiques.

[![FeatureServer](/sites/default/files/Tuto/img/featureServer/featureServer.png "FeatureServer"){: .img-center loading=lazy }](/sites/default/files/Tuto/img/featureServer/featureServer.png "FeatureServer"){: data-mediabox="gallery-lightbox" data-title="FeatureServer"}
...
Le service utilisé est défini par l'attribut default\_service du bloc [metadata]. Celui par défaut est GeoJSON. Vous pouvez néanmoins changer celui-ci pour qu'il fonctionne avec l'un des services que l'on a vu précédemment.
...
[![OpenStreetMap](/sites/default/files/Tuto/img/featureServer/osm.png "OpenStreetMap"){: .img-center loading=lazy }](/sites/default/files/Tuto/img/featureServer/osm.png  "OpenStreetMap"){: data-mediabox="gallery-lightbox" data-title="OpenStreetMap"}
