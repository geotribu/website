...
Ces questions reflètent des préoccupations courantes parmi les utilisateurs de SIG, qui s'attendent à une exactitude et à une précision rigoureuses. Mais, les 3 R : rigueur, rigueur, rigueur, cher à un de mes anciens chefs, ce n'est pas toujours… de rigueur sur nos ordinateurs.
...
- [Le constat : les calculs ne sont pas bons](https://pad.oslandia.net/wa9nxItyS3S9IJMUD2KVDw) ;
- [Fonctionnement Interne de QGIS et GEOS : comment ces outils gèrent-ils les données et les opérations géométriques](https://pad.oslandia.net/TWOJL-bJRFqj7Rp5wo6ATw) ;
- [Et les autres SIG Open Source ? Comparaisons avec GRASS et SAGA](https://pad.oslandia.net/CM1cjM-SSy668nsLhmENcQ) ;
- [Et dans les bases de données ? Comparaisons de SQL Server, Oracle et PostGIS](https://pad.oslandia.net/GKkEnTz-S-urnlgB-9jHDQ) ;
- [Utilisation de la Topologie : est-ce que la topologie peut nous sauver ?](https://pad.oslandia.net/sc_hPoyPTvyBjZC0pxZhQg) ;
- [Approche alternative : exploration de méthodes alternatives telles que l'utilisation de SFCGAL pour des calculs plus robustes](https://pad.oslandia.net/yGvdZEYrTSO7-LWXARqXnA) ;
- [Et chez la concurrence, ça se passe comment ?](https://pad.oslandia.net/7RJxGuUtQ0GxrRHvrmqWdw) ;
- [Algorithmes et Code : comment cela fonctionne-t-il ?](https://pad.oslandia.net/6FyC_frPQMmie9Vo1WeGXQ) Cette partie sera optionnelle, pour ceux ne voulant pas voir de code ;
- [La conclusion : comment arrêter de trop penser et vivre une vie meilleure !](https://pad.oslandia.net/Y_3yENWdR6GL63LDPlYmnQ)
...
- **WKT (Well-Known Text)** : Le WKT est un format texte utilisé pour représenter des objets géométriques de manière lisible par l'homme. Il est souvent utilisé pour le partage et l'affichage de données géographiques.
