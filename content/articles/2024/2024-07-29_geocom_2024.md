...
image: https://cdn.geotribu.fr/img/articles-blog-rdp/articles/2024/geocom_2024/photo.webp
license: beerware
robots: index, follow
tags:
    - geOrchestra
...
![logo geOrchestra](https://cdn.geotribu.fr/images/logos-icones/logiciels_librairies/georchestra-logo-small.png){: .img-thumbnail-left }
...
La prochaine version de geOrchestra est la version 24. Elle est prête à 99% (elle a été publiée depuis que ces rencotnres ont eu lieu).

- **Mise à jour des composants** : comme à l'accoutumé, cette version intègre des révisions des briques logicielles qui composent sont cœur : [GeoNetwork](https://www.geonetwork-opensource.org/), [GeoServer](https://geoserver.org/) et CAS notamment ;
...
geonetwork-ui est un projet de la communauté GeoNetwork dans lequel la communauté geOrchestra est largement investie, en particulier au travers de 3 mise en application :

- Le **DataHub** : interface de recherche et découverte des données référencées dans un catalogue GeoNetwork ;
...
Le DataHub bénéficie d'une communauté de plus en plus large (y compris au-delà de geOrchestra puisqu'il est question de le déployer au sein de Prodige, la Géoplateforme, [Swisstopo](https://www.swisstopo.admin.ch/fr)). Parmi les nouvelles plateformes : Métropole Européenne de Lille et l'INRAE.
...
L'outil actuel de geOrchestra dédié au suivi de la consommation des services OGC est maintenant très anciens et mériterait d'être revu. Jean Pommier ([Pi-geosolutions](https://www.pi-geosolutions.fr/)) a proposé à la communauté geOrchestra une architecture basée sur des composants logiciels qui pourraient bénéficier à d'autres usages :

- PostgreSQL enrichi de capacité à traiter des séries temporelles (ce qui permettrait facilement de consolider des indicateurs par agrégation de données sur des périodes glissantes par exemple) avec [TimescaleDB](https://www.timescale.com/) ;
- [Superset](https://superset.apache.org/) pour exploiter les données collectées et produire des indicateurs et tableaux de bord.
...
Depuis années, la communauté geOrchestra évoque l'opportunité de pouvoir exploiter des fournisseurs d'identité externes. On n'a jamais été aussi proche du but avec deux évènements proches :
...
- le travail actuel du collectif [CICCLO](https://cnig.gouv.fr/collectif-interoperabilite-et-mise-en-commun-de-a26159.html) (dans lequel se retrouvent la Géoplateforme ainsi que les communautés Prodige, Onegeo et geOrchestra) autour de l'implémentation d'[AgentConnect](https://agentconnect.gouv.fr/) voire [MonComptePro](https://moncomptepro.beta.gouv.fr/).
...
Cette présentation très didactique et démonstrative a été aussi l'occasion de présenter le contexte technologique associé et de présenter les concepts en jeux de manière simple.
...
- [CICCLO](https://cnig.gouv.fr/collectif-interoperabilite-et-mise-en-commun-de-a26159.html): Benjamin Chartier en tant que co-animateur de ce collectif (au nom de l'[Afigéo](https://www.afigeo.asso.fr/)) en a fait une présentation et a préciser les objectifs des travaux actuels portant sur l'intégration de l'API Features et d'AgentConnect aux différentes solutions des parties prenantes (Géoplateforme, geOrchestra, Onegeo et Prodige) ;
- Standards de l'OGC :
...
- Fédération d'identités : interventions déjà évoquées sur CICCLO et le nouveau module Gateway de geOrchestra ;
